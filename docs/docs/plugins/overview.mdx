...
Plugins should use `context.lib` to access diagrams-js functions instead of importing them. This prevents multiple instances and circular dependencies:
  ...
  capabilities: [
    {
      ...
      import: async (source, diagram, context) => {
        // ✅ Good: Use context.lib
        const { Node, Edge, Cluster } = context.lib;
