...
- **Importers**: Import from external formats ([Docker Compose](./docker-compose.mdx), [Kubernetes](./kubernetes.mdx), etc.)
...
Built-in plugins (JSON and SVG) are automatically registered:
...
// SVG export/import with embedded metadata
...
- `after:layout` - After Graphviz produces SVG, before PNG/JPG conversion
...
The `after:layout` hook is especially useful for SVG post-processing that should also affect PNG/JPG output. It fires after Graphviz renders the SVG string but before rasterization:

```typescript
{
  event: "after:layout",
  handler: async ({ svg, diagram, format }) => {
    // Modify SVG string before PNG/JPG conversion
...
### 1. Name Your Plugin with the `diagrams-js-plugin-` Prefix

Use the `diagrams-js-plugin-` prefix (or `@scope/diagrams-js-plugin-` for scoped packages) so the CLI and users can find it:
...
### 2. Add `diagrams-js` to package.json Keywords

Include `diagrams-js` in your `package.json` keywords to make your plugin discoverable on npm:

```json
{
  "keywords": ["diagrams-js", "diagrams", "architecture", "plugin", "terraform"]
}
```

### 3. Use Objects for Simple Plugins, Factories for Configurable Ones
...
### 6. Use Context-Provided Utilities
...
This is a minimal example of a Docker Compose plugin. See the official [Docker Compose Plugin](./docker-compose.mdx) for a complete ready-to-use plugin.
