...
## Custom CSS Classes and Data Attributes

Add CSS classes and data attributes to nodes for DOM manipulation after rendering:

```typescript
const server = diagram.add(
  Node("Server", {
    className: "highlight server-node",
    dataAttrs: { team: "backend", priority: "high" },
  }),
);
```
...
```html
<g
  class="node highlight server-node"
  data-node-id="Server"
  data-team="backend"
  data-priority="high"
>
...
You can then query and manipulate these elements in the browser:
...
const el = server.getElement();
el?.addEventListener("click", () => console.log("Clicked!"));

// Query by data attribute
const backendNodes = document.querySelectorAll('[data-team="backend"]');
```
