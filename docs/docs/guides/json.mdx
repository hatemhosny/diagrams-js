...
In addition to JSON, diagrams can be serialized through SVG. Every SVG produced by `diagram.render()` automatically embeds the full diagram metadata, making SVG a self-contained serialization format:
...
// Or use the plugin exporter (same output)
const svg2 = await diagram.export("svg");
```

SVG serialization is useful when:

- You want a single file that is both a visual diagram and a data source
- You need to distribute diagrams that can be edited later
- You are working with tools that primarily handle SVG files

To generate a clean SVG without embedded metadata:
