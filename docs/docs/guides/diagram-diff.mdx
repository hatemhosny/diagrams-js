...
1. **Fingerprint Matching**: Nodes with identical `(label, provider, service, type)` are matched directly
2. **Label Fingerprint + Edge Connectivity**: Unmatched nodes with same `(provider, service, type)` are matched using edge connectivity to disambiguate multiple candidates
3. **Simple Label Fingerprint**: Remaining unmatched nodes are matched 1:1 by `(provider, service, type)`

#### Label Change Detection

When nodes have the same `(provider, service, type)` but different labels:
...
Use the `diagrams-diff-cli` package for automatic diff generation in git workflows:
...
```typescript
import { generateDiff, diffFromGit } from "diagrams-diff-cli";

// Generate diff from two JSON files
const html = await generateDiff(beforeJSON, afterJSON, {
  format: "html",
  theme: "dark",
});

// Generate diff from git ref
const html = await diffFromGit("HEAD", "diagram.json", {
  format: "html",
});
```

For manual integration, you can also export JSON at each revision and run `Diagram.renderDiff()`.
...
- The node has different `provider`, `service`, or `type`

Nodes are only matched as `modified` when they share the same `(provider, service, type)` and have matching edge connectivity patterns. If you rename a node and significantly change its connections, it will be treated as removed + added.

To force a match as modified, ensure the node keeps the same `provider`, `service`, `type`, and similar edge connectivity.
